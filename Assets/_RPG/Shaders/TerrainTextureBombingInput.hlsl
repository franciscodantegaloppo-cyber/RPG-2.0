...
CBUFFER_START(UnityPerMaterial)
    ...
    half4 _BaseColor;
    half _Cutoff;
    float _BombingCellScale;
    float _BombingOffsetStrength;
...
float2 TerrainBombHash(float2 p)
{
...
float2 TerrainBombWeights(float2 uv)
{
    float cellScale = max(_BombingCellScale, 1.0);
    float2 weight = frac(uv / cellScale);
    return weight * weight * (3.0 - 2.0 * weight);
}

float2 TerrainBombedUV(float2 uv, float2 neighbor)
{
    float cellScale = max(_BombingCellScale, 1.0);
    float2 cell = floor(uv / cellScale) + neighbor;
    float2 hash = TerrainBombHash(cell);
    float angle = floor(hash.x * 4.0) * 1.57079632679;
    float sine;
    float cosine;
    sincos(angle, sine, cosine);
    float2 rotatedUV = mul(float2x2(cosine, -sine, sine, cosine), uv);
    return rotatedUV + (hash - 0.5) * _BombingOffsetStrength * cellScale;
}

#define SampleLayerAlbedo(i) \
    ((SAMPLE_TEXTURE2D(_Splat##i, sampler_Splat0, TerrainBombedUV(splat##i##uv, float2(0.0, 0.0))) * (1.0 - TerrainBombWeights(splat##i##uv).x) * (1.0 - TerrainBombWeights(splat##i##uv).y) + \
      SAMPLE_TEXTURE2D(_Splat##i, sampler_Splat0, TerrainBombedUV(splat##i##uv, float2(1.0, 0.0))) * TerrainBombWeights(splat##i##uv).x * (1.0 - TerrainBombWeights(splat##i##uv).y) + \
      SAMPLE_TEXTURE2D(_Splat##i, sampler_Splat0, TerrainBombedUV(splat##i##uv, float2(0.0, 1.0))) * (1.0 - TerrainBombWeights(splat##i##uv).x) * TerrainBombWeights(splat##i##uv).y + \
      SAMPLE_TEXTURE2D(_Splat##i, sampler_Splat0, TerrainBombedUV(splat##i##uv, float2(1.0, 1.0))) * TerrainBombWeights(splat##i##uv).x * TerrainBombWeights(splat##i##uv).y) * \
     half4(_DiffuseRemapScale##i.rgb, 1.0h))
