...
[InitializeOnLoad]
public static class GiantTreeOneSetup
{
    const string ScenePath = "Assets/_RPG/Scenes/SpawnVillage.unity";
    const string RootName = "giant_tree_1";
    const string ConfiguredMarker = "_GiantTree1_Configured";
    const string ModelPath =
        "Assets/MeshyImports/Meshy_Model_20260729_215935/" +
        "Meshy_AI_crea_un_arbol_gigante_0730005845_texture.fbx";
    const string MaterialPath =
        "Assets/_RPG/Materials/GiantTree1_URP.mat";
    const string TextureFolder =
        "Assets/MeshyImports/Meshy_Model_20260729_215935/";
    const float DesiredHeight = 34f;

    static GiantTreeOneSetup()
    {
    ...
    [MenuItem("RPG/World/Agregar giant_tree_1")]
    public static void EnsureInstalled()
    {
        ...
        if (existing != null)
        {
            ...
            Debug.Log("[GiantTree1] Instancia existente verificada y " +
                      "guardada. Altura=" +
        ...
        if (model == null)
        {
            Debug.LogWarning("[GiantTree1] El modelo todavía no está " +
        ...
        Debug.Log("[GiantTree1] Instalado con modelo y texturas URP. " +
                  "Posición=" + tree.transform.position +
                  ", altura=" + bounds.size.y.ToString("0.0") +
                  " m. El collider está ajustado al tronco.");
    }
    ...
    static void PlaceNearSpawn(GameObject tree)
    {
        ...
        Vector3 position = spawn != null
            ? spawn.position + spawn.right * 48f +
              spawn.forward * 30f
            : new Vector3(48f, 0f, 30f);
        tree.transform.SetPositionAndRotation(position,
    ...
    static void EnsureTrunkCollider(GameObject tree)
    {
        ...
        CapsuleCollider collider =
            tree.GetComponent<CapsuleCollider>();
        if (collider == null)
        ...
        Vector3 worldCenter = new Vector3(bounds.center.x,
            bounds.min.y + trunkWorldHeight * .5f, bounds.center.z);
        ...
        collider.height = Mathf.Max(
            trunkWorldHeight / Mathf.Max(Mathf.Abs(scale.y), .0001f),
            collider.radius * 2f);
