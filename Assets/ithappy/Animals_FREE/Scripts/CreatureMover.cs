...
namespace ithappy.Animals_FREE
{
    ...
    public class CreatureMover : MonoBehaviour
    {
        ...
        private void Update()
        {
            // CharacterController can end up disabled/inactive (scene transitions, the
            // GameObject being toggled, etc.) for a frame or more while this component's own
            // Update keeps running; Move() throws/logs in that state, so skip it until active again.
            if (!m_Controller.enabled || !m_Controller.gameObject.activeInHierarchy)
                return;
